...
1. 运行 `blutter` 命令
   ...
   **.\out**：为反编译后的文件保存路径
   ...
   <ImagePreview src="https://zzwtsy.github.io/picx-images-hosting/20240708/image.7ljvh7dy4l.webp" />

   反编译后的 out 文件夹下有如下文件

   <ImagePreview src="https://zzwtsy.github.io/picx-images-hosting/20240708/image.32hue8gt32.webp" />
...
1. reqable 手机 app 安装 Magisk 模块
   ...
   **证书管理 -> 安装根证书到本机 -> Magisk模块 -> 下载Magisk模块**
   ...
   <ImagePreview src="https://zzwtsy.github.io/picx-images-hosting/20240708/Screenshot_20240708_220215.4g4dibt6uu.webp" />
...
3. 开始 hook 函数（注意：最好不要再电脑上使用模拟器进行 hook，可能无法 hook）
   ...
   3. 在 root 模式下运行 `frida-server`（手机的 SELinux 必须处于宽容模式，否则无法运行）
      ...
      <ImagePreview src="https://zzwtsy.github.io/picx-images-hosting/20240708/image.ese3zb0um.webp" />
   ...
   5. 运行 `frida -UF -l blutter_frida.js`（要 hook 的 app 需要打开）
      ...
      <ImagePreview src="https://zzwtsy.github.io/picx-images-hosting/20240708/image.1zi53gc2ak.webp" />
